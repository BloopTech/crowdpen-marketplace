  ...
  return (
    <Card className="h-full flex flex-col hover:shadow-md transition-shadow">
      <div className="relative aspect-square overflow-hidden">
        ...
        {resource.featured && <Badge className="absolute top-2 left-2 text-xs bg-orange-500">Featured</Badge>}
      </div>

      <CardContent className="flex-1 p-4">
        <Link href={`/product/${resource.id}`}>
          <h3 className="font-semibold text-blue-600 hover:underline cursor-pointer line-clamp-1">{resource.title}</h3>
        </Link>

        <Link href={`/author/${resource.author.toLowerCase().replace(/\s+/g, "-")}`}>
          <div className="text-xs text-green-600 mt-1 mb-2 hover:underline cursor-pointer">{resource.author}</div>
        </Link>
