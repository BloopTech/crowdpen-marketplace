            ...
            const html = await pretty(
              await render(
                <OrderConfirmationEmail
                  ...
                  currencyCode={
                    String(locked.paid_currency || locked.currency || "USD")
                      .trim()
                      .toUpperCase()
                  }
                />
      ...
      try {
        if (!alreadySuccessful) {
          ...
          const currencyCode = String(
            locked.paid_currency || locked.currency || "USD"
          )
            .trim()
            .toUpperCase();
          ...
          for (const saleKey of Object.keys(vendorSales)) {
            ...
            try {
              ...
              await sendEmail({
                ...
                html: saleHtml,
                text: `You made a sale! ${productName}. You earned ${earned.toFixed(2)} ${currencyCode}. Order: ${locked.order_number}`,
              });
            } catch (itemErr) {
              await reportError(itemErr, {
                ...
                status: 200,
                requestId,
                tag: "paystack_webhook_vendor_sale_email",
        ...
        await reportError(e, {
          ...
          status: 200,
          requestId,
          tag: "paystack_webhook_vendor_sale_emails",
