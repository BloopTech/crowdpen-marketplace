...
export default function AdminAnalyticsPage() {
  ...
  const categoryBreakdownQuery = useQuery({
    ...
    queryFn: () => {
      ...
      if (qs.from) sp.set("from", qs.from);
      if (qs.to) sp.set("to", qs.to);
      return fetchJson(`/api/admin/analytics/category-breakdown?${sp.toString()}`);
    },
  });
  ...
  const topProductsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (qs.from) sp.set("from", qs.from);
      if (qs.to) sp.set("to", qs.to);
      return await fetchJson(`/api/admin/analytics/top-products?${sp.toString()}`);
    },
  });
  ...
  const topMerchantsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (qs.from) sp.set("from", qs.from);
      if (qs.to) sp.set("to", qs.to);
      return await fetchJson(`/api/admin/analytics/top-merchants?${sp.toString()}`);
    },
  });
  ...
  const chartData = revenueData.map((r) => ({
    ...
    crowdpenFee: Number(r.crowdpenFee || 0) || 0,
    startbuttonFee: Number(r.startbuttonFee || 0) || 0,
    creatorPayout: Number(r.creatorPayout || 0) || 0,
  }));

  const chartConfig = {
    revenue: { label: "Revenue", color: "hsl(var(--chart-1))" },
    crowdpenFee: { label: "Crowdpen Fee", color: "hsl(var(--chart-2))" },
    startbuttonFee: { label: "Gateway Fee", color: "hsl(var(--chart-3))" },
    creatorPayout: { label: "Creator Payout", color: "hsl(var(--chart-4))" },
  };
          ...
          {summaryQuery.isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="rounded border border-border p-3">
                ...
                <div className="text-base font-semibold">{fmtMoney(summary.grossRevenue)}</div>
              </div>
              <div className="rounded border border-border p-3">
                <div className="text-xs text-muted-foreground">AOV</div>
                <div className="text-base font-semibold">{fmtMoney(summary.aov)}</div>
              </div>
              ...
              <div className="rounded border border-border p-3">
                ...
                <div className="text-base font-semibold">{fmtMoney(summary.refundRevenue)}</div>
              </div>
              <div className="rounded border border-border p-3">
                <div className="text-xs text-muted-foreground">Net Revenue</div>
                <div className="text-base font-semibold">{fmtMoney(summary.netRevenue)}</div>
              </div>
              <div className="rounded border border-border p-3">
                <div className="text-xs text-muted-foreground">Payout Coverage</div>
                <div className="text-base font-semibold">
              ...
              <Table>
                ...
                <TableBody>
                  {topProducts.map((p) => (
                    <TableRow key={p.id}>
                      <TableCell>
                        <div className="font-medium max-w-[240px] truncate" title={p.title}>
                          {p.title}
                        </div>
                        <div className="text-xs text-muted-foreground">
                      ...
                      <TableCell>
                        {p.merchantPenName || p.merchantName || p.merchantId || "-"}
                      </TableCell>
                ...
                <TableBody>
                  {topMerchants.map((m) => (
                    <TableRow key={m.id}>
                      ...
                      <TableCell className="text-right">{fmtMoney(m.revenue)}</TableCell>
                      <TableCell className="text-right">
          ...
          <CardContent>
            {timeToPayoutQuery.isLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <div className="text-sm text-muted-foreground">P50 (hours)</div>
                  <div className="text-2xl font-semibold">
                    {Number(timeToPayout.summary.p50Hours || 0).toFixed(1)}
                  </div>
                </div>
                <div>
                  <div className="text-sm text-muted-foreground">P90 (hours)</div>
                  <div className="text-2xl font-semibold">
                ...
                <div>
                  ...
                  <div className="text-2xl font-semibold">
                    {Number(timeToPayout.summary.count || 0).toLocaleString("en-US")}
                  </div>
                </div>
              </div>
            ) : (
              <div className="text-sm text-muted-foreground">No payout allocation data.</div>
            )}
          </CardContent>
        </Card>
      </div>
                ...
                <TableBody>
                  {paymentStatusRows.map((r) => (
                    <TableRow key={r.paymentStatus}>
                      ...
                      <TableCell className="text-right">{fmtMoney(r.orderTotal)}</TableCell>
                    </TableRow>
                  ))}
            ...
            {customersQuery.isLoading ? (
              ...
              <div className="space-y-3">
                <div className="rounded border border-border p-3">
                  ...
                  <div className="text-base font-semibold">
                    {Number(customers.uniqueBuyers || 0).toLocaleString("en-US")}
                  </div>
                </div>
                <div className="rounded border border-border p-3">
                  <div className="text-xs text-muted-foreground">Repeat Buyers</div>
                  <div className="text-base font-semibold">
                    {Number(customers.repeatBuyers || 0).toLocaleString("en-US")}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {new Intl.NumberFormat("en-US", {
                      style: "percent",
                ...
                <div className="rounded border border-border p-3">
                  <div className="text-xs text-muted-foreground">First-time Buyers</div>
                  <div className="text-base font-semibold">
                    {Number(customers.firstTimeBuyers || 0).toLocaleString("en-US")}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {Number(customers.ordersPerBuyer || 0).toFixed(2)} orders/buyer
                  </div>
                </div>
              </div>
            ) : (
                ...
                <TableBody>
                  {paymentMethods.map((r) => (
                    <TableRow key={r.paymentMethod}>
                      ...
                      <TableCell className="text-right">{fmtMoney(r.orderTotal)}</TableCell>
                      <TableCell className="text-right">{fmtMoney(r.discountTotal)}</TableCell>
                    </TableRow>
                  ))}
              ...
              <div className="space-y-3">
                {couponTotals && (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded border border-border p-3">
                      <div className="text-xs text-muted-foreground">Coupon Orders</div>
                      <div className="text-base font-semibold">
                        {Number(couponTotals.orderCount || 0).toLocaleString("en-US")}
                      </div>
                    </div>
                    <div className="rounded border border-border p-3">
                ...
                <TableBody>
                  {cohortRows.map((r) => (
                    <TableRow key={r.cohortPeriod}>
                      <TableCell>
                        {r.cohortPeriod
                          ? new Date(r.cohortPeriod).toLocaleDateString("en-US", { timeZone: "UTC" })
                          : "-"}
                      </TableCell>
                      <TableCell className="text-right">
                ...
                <TableBody>
                  {inventoryRiskRows.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell>
                        <div className="font-medium max-w-[220px] truncate" title={r.title}>
                          {r.title}
                        </div>
                        <div className="text-xs text-muted-foreground">
                      ...
                      <TableCell>
                        {r.merchantPenName || r.merchantName || r.merchantId || "-"}
                      </TableCell>
                      <TableCell className="text-right">
                        {r.stock == null ? "-" : Number(r.stock).toLocaleString("en-US")}
                      </TableCell>
                      <TableCell className="text-right">
                        {Number(r.unitsSoldWindow || 0).toLocaleString("en-US")}
                      </TableCell>
                      <TableCell className="text-right">
                        {r.coverageDays == null ? "-" : Number(r.coverageDays).toFixed(1)}
                      </TableCell>
                    </TableRow>
                  ))}
